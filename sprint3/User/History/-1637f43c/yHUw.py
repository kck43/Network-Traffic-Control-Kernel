...
To ensure that both timelines start from the first `sys_enter_sendto` timestamp, you should adjust all timestamps by subtracting the first `sys_enter_sendto` timestamp. This effectively normalizes the time axis so that the first `sys_enter_sendto` event occurs at time `0`, and all other timestamps are shifted accordingly.
...
### What This Fix Does:
1. **Computes the first `sys_enter_sendto` timestamp** (`start_time`).
2. **Subtracts `start_time` from all timestamps**, so the first `sys_enter_sendto` event appears at `0 ns`.
3. **Ensures both plots share the same x-axis range**, preventing artificial shifts.

Now, the graph will properly show `sys_enter_sendto` starting at `0 ns`, with `net_dev_xmit` timestamps appearing in relation to that reference point.
